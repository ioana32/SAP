...
int main()
{
	...
	int result = ECDSA_sign(0, (const unsigned char*)in_data, sizeof(in_data),
							(unsigned char*)signature, (unsigned int*)&sig_len, key);
	...
	if (s != NULL) {
		if (d2i_ECDSA_SIG(&s, (const unsigned char**)&signature, sig_len) != NULL) {  // decode the DER encoded Signature into a ECDSA_SIG structure
			...
			// in_data[0] = 0xf6; // generate signature invalidation
