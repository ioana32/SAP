...
int sign_it(const byte* msg, size_t mlen, byte** sig, size_t* slen, EVP_PKEY* pkey)
{
	...
	*slen = 0;
	...
	do
	{
		...
		size_t req = 0;
		rc = EVP_DigestSignFinal(ctx, NULL, &req); // only the length of the signature is returned by req
		assert(rc == 1);
		if (rc != 1) {
		...
		*slen = req;
		rc = EVP_DigestSignFinal(ctx, *sig, slen); // the signature buffer is populated by the signature content (bytes)
		assert(rc == 1);
		if (rc != 1) {
...
int make_keys(EVP_PKEY** skey, EVP_PKEY** vkey)
{
	...
	do
	{
		const EVP_MD* md = EVP_get_digestbyname(hn); // initialize the MD structure accroding to the message digest algo name
		assert(md != NULL);
		if (md == NULL) {
			printf("EVP_get_digestbyname failed, error 0x%lx\n", ERR_get_error());
			break; /* failed */
